...
for (var k = 0; k < colsys.length; k++) {
  ...
  colorf += ".btn-" + color_name + ":active{background:" + color_deepcolor + "!important}";

  colorj += ".btnl-" + color_name + ",.btnk-" + color_name + "{color: " + color_lwcolor + "!important}";

  colork += ".inp-h-" + color_name + ":active,.inp-h-" + color_name + ":active i{border-color:" + color_color + "; background-color:" + color_deeplightcolor + ";color:" + color_color + "}";
}
...
for (var l = 0; l < sizefont.size.length; l++) {
  msize += "//" + sizefont.size[l] + "<br>";

  //通用高度宽度
  msize += ".btn,.btn.ico,.inp,.ico-inp,.cka,.ckb,.page li{height:" + sizefont.size[l] + "px!important}<br>";
  //纯图片，lab按钮图标为正方形小1px
  msize += " .btn.ico i,.btn.ico em,.btn.lab i{width:" + sizefont.sizef[l] + "px;height:" + sizefont.sizef[l] + "px;line-height:" + sizefont.sizef[l] + "px}<br>";
  //将纯图片按钮和输入框图标定义为方块
  msize += ".btn.ico,.ico-inp i{width:" + sizefont.size[l] + "px}<br>";
  msize += ".select i{width:" + sizefont.sizes[l] + "px}<br>";
  //修正输入框图标居中
  msize += ".ico-inp i,.ckb span,.select i,.page li{line-height:" + sizefont.size[l] + "px}<br>";
  //图标文字大小统一声明
  msize += ".btn i,.ico-inp i,.select i{font-size:" + sizefont.fontb[l] + "px}<br>";
  msize += ".btn em,.inp,.ckb span,.page li{font-size:" + sizefont.fonta[l] + "px}<br>";
  //按钮内边距  
  msize += ".btn{padding:" + btns.btnpadtb[l] + "px " + btns.btnpadlr[l] + "px}<br>";
  msize += ".btn i{margin-right:" + btns.btnimagr[l] + "px}<br>";
  //lab按钮图标位置修正
  msize += ".btn.lab i{margin:-" + btns.btnpadtb[l] + "px " + btns.btnpadlr[l] + "px -" + btns.btnpadtb[l] + "px -" + btns.btnpadlr[l] + "px}<br>";
  //修正右侧下拉类图标边距
  msize += ".btn.d{padding:" + btns.btnpadtb[l] + "px " + btns.btndpad[l] + "px!important}<br>";
  //纯图标输入框左右侧内边距纠正
  msize += ".ico-inp .inp{padding-left:" + sizefont.size[l] + "px}<br>";
  msize += ".ico-inp.ico-r .inp{padding-right:" + sizefont.size[l] + "px;padding-left:8px}<br>";
  //方块图标输入框左右侧内边距纠正
  msize += ".ico-inp.block .inp{padding-left:" + sizefont.sizee[l] + "px}<br>";
  msize += ".ico-inp.block.ico-r .inp{padding-right:" + sizefont.sizee[l] + "px; padding-left:8px}<br>";
  //右侧圆形图标输入框图标
  msize += ".ico-inp.ico-r i.o{width:" + sizefont.sizeb[l] + "px;height:" + sizefont.sizeb[l] + "px;line-height:" + sizefont.sizeb[l] + "px}<br>";
  //选择框
  msize += ".cka{padding:" + btns.btnpadtb[l] + "px 0}<br>";
  msize += ".ckb span{padding:0 " + btns.btnpadlr[l] + "px}<br>";
  //select
  msize += ".select .inp{padding-right:" + sizefont.sizeb[l] + "px}<br>";
  //分页按钮
  msize += ".page li{min-width:" + sizefont.size[l] + "px}<br>";
  msize += "<br><br>";
}
